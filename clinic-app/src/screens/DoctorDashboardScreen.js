...
export default function DoctorDashboardScreen({ navigation }) {
  ...
  const resolveDoctorProfile = (doctors = []) => {
    ...
    return (
      doctors.find((doc) => String(doc?.userId || '') === String(userId || '')) ||
      doctors.find((doc) => String(doc?.name || '').trim().toLowerCase() === normalizedName) ||
      null
    );
  };
  ...
  const fetchDoctorProfile = async () => {
    try {
      ...
      const profile = resolveDoctorProfile(response.data?.doctors || []);
  ...
  const fetchAppointments = async (profile) => {
    try {
  ...
  const loadDashboardData = async () => {
    ...
    await fetchAppointments(profile);
  };
